...
@OnlyIn(Dist.CLIENT)
public class DeathEggRobotRenderer {
	public static class ModelRegisterHandler {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
		public void registerModels(ModelRegistryEvent event) {
			RenderingRegistry.registerEntityRenderingHandler(DeathEggRobotEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modeldeath_egg_robot(), 2.8000000000000003f) {
					{
						this.addLayer(new GlowingLayer<>(this));
					}
					@Override
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("sonicraft:textures/death_egg_robot.png");
					}
				};
			});
		}
	...
	private static class GlowingLayer<T extends Entity, M extends EntityModel<T>> extends LayerRenderer<T, M> {
		...
		public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing,
			...
			IVertexBuilder ivertexbuilder = bufferIn
					.getBuffer(RenderType.getEyes(new ResourceLocation("sonicraft:textures/death_egg_robot_glow.png")));
	...
	public static class Modeldeath_egg_robot extends EntityModel<Entity> {
		...
		public void setRotationAngles(Entity e, float f, float f1, float f2, float f3, float f4) {
			...
			this.left_hand.rotateAngleY = f2 / 20.f;
			this.left_arm.rotateAngleX = MathHelper.cos(f * 1.0F) * -1.0F * f1;
			this.right_leg.rotateAngleX = MathHelper.cos(f * 1.0F) * 1.0F * f1;
			this.right_hand.rotateAngleY = f2 / 20.f;
		}
	}
}
