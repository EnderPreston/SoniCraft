...
package net.mcreator.sonicraft.world.dimension;
...
import net.mcreator.sonicraft.procedures.MobiusPlayerLeavesDimensionProcedure;
import net.mcreator.sonicraft.procedures.MobiusPlayerEntersDimensionProcedure;
...
public class MobiusDimension extends SonicraftModElements.ModElement {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(ChaostoneBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("sonicraft:mobius"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("mobius_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("mobius_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("mobius_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new MobiusItem().setRegistryName("mobius"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			for (int i = 0; i < 4; i++) {
				...
				world.addParticle(SuperStarsParticle.particle, px, py, pz, vx, vy, vz);
			}
		}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("sonicraft:mobius"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("sonicraft:mobius")));
				} else {
					entity.func_242279_ag();
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getFrom() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("sonicraft:mobius"))) {
			{
				...
				MobiusPlayerLeavesDimensionProcedure.executeProcedure($_dependencies);
			}
		}
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("sonicraft:mobius"))) {
			{
				Map<String, Object> $_dependencies = new HashMap<>();
				$_dependencies.put("x", x);
				$_dependencies.put("y", y);
				$_dependencies.put("z", z);
				$_dependencies.put("world", world);
				MobiusPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
